    ...
    for (const p of positions) {
      try {
        ...
        if (!filled || filled.status !== "filled") {
          console.log(`  flatten ${p.symbol}: close not confirmed (status ${filled ? filled.status : "no-order"}) — leaving open, will retry next run.`);
          continue; // no phantom row, no exited flag — next run re-attempts / detects
        }
        ...
        state.realizedPnl += pnl;
        logTrade([now.date, hhmm(now.min), p.symbol, side, "EXIT", qty, exitPx.toFixed(2), "", "eod-flatten", ""]);
        console.log(`  EOD flatten ${p.symbol} ${qty}sh @ ${exitPx.toFixed(2)} P/L $${pnl.toFixed(2)} | day $${state.realizedPnl.toFixed(2)}`);
        await sendSms(
          `PAPER ${TAG} EXIT ${p.symbol} eod @ $${exitPx.toFixed(2)} P/L $${pnl.toFixed(2)}`,
        );
      } catch (e) {
        console.log(`  flatten ${p.symbol} failed: ${e.message}`);
      }
    }
    saveState(state);
    return;
